    ...
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-[#243244] bg-[#111827] p-5">
        ...
        <form onSubmit={submitMovement} className="manual-entry-form mt-4">
          <label><span className="label-dark">Tipo</span><select id="cash-movement-type" name="cash-movement-type" value={movement.type} onChange={(event) => setMovementField('type', event.target.value)} className="input-dark" autoComplete="off"><option value="income">Ingreso</option><option value="expense">Salida / gasto</option><option value="withdrawal">Retiro</option></select></label>
          <label><span className="label-dark">Categoria</span><select id="cash-movement-category" name="cash-movement-category" value={movement.category} onChange={(event) => setMovementField('category', event.target.value)} className="input-dark" autoComplete="off">{movementCategories.map((category) => <option key={category}>{category}</option>)}</select></label>
          <label><span className="label-dark">Metodo de pago</span><select id="cash-movement-method" name="cash-movement-method" value={movement.method} onChange={(event) => setMovementField('method', event.target.value)} className="input-dark" autoComplete="off">{paymentMethods.map((method) => <option key={method}>{method}</option>)}</select></label>
          <label><span className="label-dark">Fecha del movimiento</span><input id="cash-movement-date" name="cash-movement-date" type="date" value={movement.movementDate} onChange={(event) => setMovementField('movementDate', event.target.value)} className="input-dark"  autoComplete="off" /></label>
          <label><span className="label-dark">Monto</span><input id="cash-movement-amount" name="cash-movement-amount" type="number" value={movement.amount} onChange={(event) => setMovementField('amount', event.target.value)} className="input-dark"  autoComplete="off" /></label>
          <label className="manual-entry-wide"><span className="label-dark">Concepto</span><input id="cash-movement-concept" name="cash-movement-concept" value={movement.concept} onChange={(event) => setMovementField('concept', event.target.value)} className="input-dark" placeholder="Ej. Pago de internet, envio a cliente, compra operativa..."  autoComplete="off" /></label>
          <label><span className="label-dark">Destino / hacia donde fue</span><input id="cash-movement-destination" name="cash-movement-destination" value={movement.destination} onChange={(event) => setMovementField('destination', event.target.value)} className="input-dark" placeholder="Proveedor, cliente, sucursal..."  autoComplete="off" /></label>
          <label><span className="label-dark">Mensajero / responsable</span><input id="cash-movement-messenger" name="cash-movement-messenger" value={movement.messenger} onChange={(event) => setMovementField('messenger', event.target.value)} className="input-dark" placeholder="Nombre o ruta"  autoComplete="off" /></label>
          <label><span className="label-dark">Referencia</span><input id="cash-movement-reference" name="cash-movement-reference" value={movement.reference} onChange={(event) => setMovementField('reference', event.target.value)} className="input-dark" placeholder="Recibo, transferencia, comprobante"  autoComplete="off" /></label>
          <label><span className="label-dark">Canal</span><input id="cash-movement-channel" name="cash-movement-channel" value={movement.channel} onChange={(event) => setMovementField('channel', event.target.value)} className="input-dark" placeholder="Caja, banco, mensajeria..."  autoComplete="off" /></label>
          <label className="manual-entry-notes"><span className="label-dark">Notas</span><textarea id="cash-movement-notes" name="cash-movement-notes" value={movement.notes} onChange={(event) => setMovementField('notes', event.target.value)} className="input-dark min-h-20 resize-y" placeholder="Detalle del pago, quien autorizo, ruta, observaciones..."  autoComplete="off" /></label>
      ...
      <Modal open={openModal} onClose={() => setOpenModal(false)} title="Abrir caja" size="md" footer={<div className="flex justify-end gap-2"><Button variant="ghost" onClick={() => setOpenModal(false)}>Cancelar</Button><Button variant="success" icon={Unlock} onClick={handleOpen}>Abrir caja</Button></div>}>
        <div className="grid gap-3">
          <label><span className="label-dark">Sucursal</span><select id="cash-branch" name="cash-branch" value={openForm.branchId} onChange={(event) => selectBranch(event.target.value)} className="input-dark" autoComplete="off"><option value="">Sucursal principal</option>{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label>
          <label><span className="label-dark">Caja</span><input id="cash-name" name="cash-name" value={openForm.cashName} onChange={(event) => setOpenField('cashName', event.target.value)} className="input-dark"  autoComplete="off" /></label>
          <label><span className="label-dark">Cajero</span><input id="cash-cashier" name="cash-cashier" value={openForm.cashier} onChange={(event) => setOpenField('cashier', event.target.value)} className="input-dark"  autoComplete="off" /></label>
          <label><span className="label-dark">Monto inicial</span><input id="cash-opening-amount" name="cash-opening-amount" type="number" value={openForm.amount} onChange={(event) => setOpenField('amount', event.target.value)} className="input-dark"  autoComplete="off" /></label>
        </div>
      </Modal>
      ...
      <Modal open={closeModal} onClose={() => setCloseModal(false)} title="Confirmar cierre de caja" description="Revise el efectivo contado antes de cerrar. El cierre es irreversible hasta una nueva apertura." size="md" footer={<div className="flex justify-end gap-2"><Button variant="ghost" onClick={() => setCloseModal(false)}>Cancelar</Button><Button variant="danger" icon={Lock} onClick={handleClose}>Confirmar cierre</Button></div>}>
        <div className="space-y-3">
          ...
          <label><span className="label-dark">Efectivo contado al cierre</span><input id="cash-counted" name="cash-counted" type="number" value={counted} onChange={(event) => setCounted(event.target.value)} className="input-dark"  autoComplete="off" /></label>
      ...
      <Modal open={historyModal} onClose={() => setHistoryModal(false)} title="Historial de movimientos de caja" size="xl">
        <div className="space-y-4">
          <div className="grid gap-3 md:grid-cols-2">
            <label><span className="label-dark">Desde</span><input id="cashdesk-history-from" name="historyFrom" type="date" value={historyRange.from} onChange={(event) => setHistoryRange((state) => ({ ...state, from: event.target.value }))} className="input-dark" autoComplete="off" /></label>
            <label><span className="label-dark">Hasta</span><input id="cashdesk-history-to" name="historyTo" type="date" value={historyRange.to} onChange={(event) => setHistoryRange((state) => ({ ...state, to: event.target.value }))} className="input-dark" autoComplete="off" /></label>
          </div>
