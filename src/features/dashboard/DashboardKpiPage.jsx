          ...
          <>
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-[1.2fr_.7fr_.7fr_.6fr_.6fr]">
              <label className="rounded-lg border border-white/10 bg-black/20 px-3 py-2">
                ...
                <input id="kpi-search" name="kpiSearch" value={filters.query} onChange={(event) => setFilter('query', event.target.value)} className="w-full bg-transparent text-sm outline-none" placeholder="Cliente, producto, factura, metodo, estado..."  autoComplete="off" />
              </label>
              <label htmlFor="kpi-date-from"><span className="label-dark flex items-center gap-2"><CalendarDays size={14} /> Desde</span><input id="kpi-date-from" name="kpiDateFrom" type="date" value={filters.dateFrom} onChange={(event) => setFilter('dateFrom', event.target.value)} className="input-dark"  autoComplete="off" /></label>
              <label htmlFor="kpi-date-to"><span className="label-dark">Hasta</span><input id="kpi-date-to" name="kpiDateTo" type="date" value={filters.dateTo} onChange={(event) => setFilter('dateTo', event.target.value)} className="input-dark"  autoComplete="off" /></label>
              <label htmlFor="kpi-time-from"><span className="label-dark">Hora inicio</span><input id="kpi-time-from" name="kpiTimeFrom" type="time" value={filters.timeFrom} onChange={(event) => setFilter('timeFrom', event.target.value)} className="input-dark"  autoComplete="off" /></label>
              <label htmlFor="kpi-time-to"><span className="label-dark">Hora fin</span><input id="kpi-time-to" name="kpiTimeTo" type="time" value={filters.timeTo} onChange={(event) => setFilter('timeTo', event.target.value)} className="input-dark"  autoComplete="off" /></label>
            </div>
