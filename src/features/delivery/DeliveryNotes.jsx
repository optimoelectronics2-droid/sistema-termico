    ...
    <div className="space-y-0">
      <section className="module-header">
        ...
        <div className="grid gap-3 lg:grid-cols-[260px_1fr_220px]">
          <select id="delivery-customer" value={form.customerId} onChange={(event) => setForm((state) => ({ ...state, customerId: event.target.value }))} className="input-dark" aria-label="delivery-customer" autoComplete="off" name="delivery-customer">
            <option value="">Seleccione cliente</option>
            {customers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
          </select>
          <select id="delivery-product" onChange={(event) => { addProduct(event.target.value); event.target.value = '' }} className="input-dark" aria-label="delivery-product" autoComplete="off" name="delivery-product">
        ...
        <textarea id="delivery-notes" value={form.notesCustomer} onChange={(event) => setForm((state) => ({ ...state, notesCustomer: event.target.value }))} className="input-dark mt-3 min-h-20" placeholder="Observaciones"  autoComplete="off"  name="delivery-notes" />
        <div className="mt-4 space-y-2">
          {form.items.map((item, index) => (
            <div key={`${item.productId}-${index}`} className="grid gap-2 rounded-lg border border-white/10 bg-white/[0.035] p-3 text-sm md:grid-cols-[1fr_100px_120px_40px]">
              ...
              <input id={`delivery-item-qty-${index}`} type="number" min="1" value={item.quantity} onChange={(event) => setForm((state) => ({ ...state, items: state.items.map((line, lineIndex) => lineIndex === index ? { ...line, quantity: Number(event.target.value) } : line) }))} className="input-dark" aria-label={`delivery-item-qty-${index}`}  autoComplete="off"  name="deliverynotes-field" />
