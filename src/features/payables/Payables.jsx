...
function PayableForm({ draft, setDraft, suppliers, editing = false }) {
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2">
      <label><span className="label-dark">Proveedor</span><select id="payable-supplier" value={draft.supplierId} onChange={(event) => set('supplierId', event.target.value)} className="input-dark" autoComplete="off" name="payable-supplier">{suppliers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
...
function Input({ label, value, onChange, type = 'text', step, min, id }) {
  const inputId = id || `payable-${label ? label.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') : 'field'}`
  return <label><span className="label-dark">{label}</span><input id={inputId} type={type} step={step} min={min} value={value || ''} onChange={(event) => onChange(event.target.value)} className="input-dark" autoComplete="off" name={inputId} /></label>
}

function Select({ label, value, onChange, options, id }) {
  const selectId = id || `payable-${label ? label.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') : 'field'}`
  return <label><span className="label-dark">{label}</span><select id={selectId} value={value} onChange={(event) => onChange(event.target.value)} className="input-dark" autoComplete="off" name={selectId}>{options.map((option) => <option key={option}>{option}</option>)}</select></label>
}
