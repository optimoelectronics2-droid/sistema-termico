    ...
    <div className="space-y-5">
      ...
      <section>
        ...
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <label className="flex items-center gap-2 pt-6 text-sm"><input id="require-open-register" type="checkbox" checked={companyDraft.requireOpenRegister} onChange={(e) => setCompanyDraft((state) => ({ ...state, requireOpenRegister: e.target.checked }))}  autoComplete="off"  name="require-open-register" /> Requerir caja abierta</label>
        </div>
      ...
      <section>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-4">
          <Field label="Modo por defecto"><select id="fiscal-default-mode" value={fiscalDraft.defaultMode} onChange={(event) => setFiscalDraft((state) => ({ ...state, defaultMode: event.target.value }))} className="input-dark" autoComplete="off" name="fiscal-default-mode"><option value="normal">Normal sin NCF</option><option value="ncf">NCF</option><option value="ecf">e-CF DGII</option></select></Field>
          <Field label="Ambiente e-CF"><select id="fiscal-ecf-environment" value={fiscalDraft.ecfEnvironment} onChange={(event) => setFiscalDraft((state) => ({ ...state, ecfEnvironment: event.target.value }))} className="input-dark" autoComplete="off" name="fiscal-ecf-environment"><option value="certification">Certificacion</option><option value="production">Produccion</option></select></Field>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <div>
          ...
          <textarea id="categories" value={categoryText} onChange={(e) => setCategoryText(e.target.value)} className="input-dark mt-4 min-h-24"  autoComplete="off"  name="categories" />
          ...
          <div className="space-y-2">
            {taxSequences.map((sequence) => (
              <div key={sequence.id} className="grid gap-2 rounded-lg border border-white/10 bg-white/[0.035] p-2 md:grid-cols-5">
                ...
                <input id={`seq-next-${sequence.id}`} type="number" value={sequence.next} onChange={(e) => updateTaxSequence({ type: sequence.id, next: Number(e.target.value) })} className="input-dark" aria-label={`seq-next-${sequence.id}`}  autoComplete="off"  name="settingspage-field" />
                <input id={`seq-limit-${sequence.id}`} type="number" value={sequence.limit} onChange={(e) => updateTaxSequence({ type: sequence.id, limit: Number(e.target.value) })} className="input-dark" aria-label={`seq-limit-${sequence.id}`}  autoComplete="off"  name="settingspage-field" />
                <input id={`seq-expires-${sequence.id}`} type="date" value={sequence.expiresAt} onChange={(e) => updateTaxSequence({ type: sequence.id, expiresAt: e.target.value })} className="input-dark" aria-label={`seq-expires-${sequence.id}`}  autoComplete="off"  name="settingspage-field" />
                <label className="flex items-center gap-2 text-sm"><input id={`seq-enabled-${sequence.id}`} type="checkbox" checked={sequence.enabled} onChange={(e) => updateTaxSequence({ type: sequence.id, enabled: e.target.checked })}  autoComplete="off"  name="settingspage-field" /> Activa</label>
              </div>
            ))}
          </div>
        </div>
      ...
      <section>
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          <Field label="Tamaño etiqueta por defecto"><select id="settingspage-label-size" value={companyDraft.defaultLabelSize || '3x2'} onChange={(e) => setCompanyDraft((s) => ({ ...s, defaultLabelSize: e.target.value }))} className="input-dark" autoComplete="off" name="settingspage-label-size">{Object.entries(LABEL_SIZES).filter(([id]) => !['letter','a4','58mm','80mm'].includes(id)).map(([id, dim]) => <option key={id} value={id}>{dim.name}</option>)}</select></Field>
          <Field label="Resolucion DPI"><select id="label-dpi" value={String(companyDraft.labelDpi || 203)} onChange={(e) => setCompanyDraft((s) => ({ ...s, labelDpi: Number(e.target.value) }))} className="input-dark" autoComplete="off" name="label-dpi">{DPI_VALUES.map(d => <option key={d} value={String(d)}>{d} DPI</option>)}</select></Field>
          <Field label="Incluir precio"><select id="label-show-price" value={String(companyDraft.labelShowPrice ?? true)} onChange={(e) => setCompanyDraft((s) => ({ ...s, labelShowPrice: e.target.value === 'true' }))} className="input-dark" autoComplete="off" name="label-show-price"><option value="true">Si</option><option value="false">No</option></select></Field>
          <Field label="Metodo de impresion"><select id="label-print-mode" value={companyDraft.labelPrintMode || 'browser'} onChange={(e) => setCompanyDraft((s) => ({ ...s, labelPrintMode: e.target.value }))} className="input-dark" autoComplete="off" name="label-print-mode"><option value="browser">PDF vectorial</option><option value="zpl">ZPL (Zebra, Xprinter, Rongta)</option><option value="epl">EPL (Zebra older)</option><option value="tspl">TSPL (TSC)</option><option value="cpcl">CPCL (Honeywell, Citizen)</option><option value="escpos">ESC/POS (Epson, Bixolon, Star)</option><option value="usb">ZPL WebUSB directo</option><option value="escpos-usb">ESC/POS WebUSB directo</option><option value="png">Imagen PNG</option></select></Field>
        </div>
...
function Input({ label, value, onChange, type = 'text', id }) {
  ...
  return <label><span className="label-dark">{label}</span><input id={inputId} type={type} value={value || ''} onChange={(e) => onChange(e.target.value)} className="input-dark"  autoComplete="off"  name="settingspage-field" /></label>
}
function List({ items }) {
